...
def test_render_a_bot_and_some_cups():
  ...
  env = Environment()
  id = env.create(Bot(x=-1000, y=0, mass=1, logic=move_forward))  
  env.attach(id, Host(move_forward))
  env.create(stw.Cup(x=0, y=70, color=stw.CupColor.RED))
...
def test_drive_a_bot_with_forward_velocity():
  ...
  env = Environment()
  id = env.create(Bot(x=-1000, y=0, mass=1, logic=drive_n_turn))
