...
Expected<> arena::component::detail::Storage_impl::store(Expected<entt::entity> this_entity_xp, Environment& environment, entt::entity target) {
  using enum Error;
...
Expected<> arena::component::detail::Storage_impl::remove(Expected<entt::entity> this_entity_xp, Environment& environment, entt::entity target) {
  using namespace ltl;
  using enum Error;

  return expected(this_entity_xp, environment.try_get<ownership_t>(target))
    .and_then(unzip(_((this_entity, ownership),
            (ownership.owner == this_entity
            ? ownership.owner = entt::null, expected()
            : unexpected(NOT_OWNED)))))
    .transform([&]() { m_count--; });
}
